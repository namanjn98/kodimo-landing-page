...
1. Ensure `.mcp.json` in the project root contains:
   ```json
   {
     "mcpServers": {
       "figma": {
...
3. Claude Code will handle OAuth authentication automatically when the Figma MCP server is first accessed.

After setup, the user will need to restart Claude Code. Inform them so they can continue with Step 1 on restart.
